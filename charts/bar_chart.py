...
class BarChart(BaseChart):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        super(BarChart, self).__init__(parent=parent, *args, **kwargs)
        self.parent = parent
        ...
        if not self.parent.df.empty:
            ...
            if self.parent.chart_tool.selected_bar == "grouped":
                for i, col in enumerate(cols):
                    ...
                    rect = self.canvas.axs.bar(rou[i], y_series, width=self.parent.chart_tool.width,
                                               color=colors[i], label=data_labels[i], yerr=y_err_series_list[i],
                                               ecolor=self.parent.chart_tool.err_bar_color, align="center")
                    if isinstance(x_series[0], str):
                        self.canvas.axs.set_xticks(rou[i]-(col_num-1)*self.parent.chart_tool.width/2, x_series)
                ...
                bottom = np.zeros(self.parent.df.shape[0])
                for i, col in enumerate(cols):
                    y_series = self.parent.df[col].values
                    rect = self.canvas.axs.bar(x_series, y_series, width=self.parent.chart_tool.width,
                                               color=colors[i], label=data_labels[i], yerr=y_err_series_list[i],
                                               ecolor=self.parent.chart_tool.err_bar_color, bottom=bottom)
                    bottom += y_series
                    if self.parent.chart_tool.bar_value_label_checked:
                        if len(self.parent.chart_tool.bar_value_label_series) < col_num:
                            self.parent.chart_tool.bar_value_label_series.extend(
                ...
                bottom = np.zeros(self.parent.df.shape[0])
                for i, col in enumerate(cols):
                    rect = self.canvas.axs.bar(x_series, value_props[:, i], width=self.parent.chart_tool.width,
                                               color=colors[i], label=data_labels[i], yerr=y_err_series_list[i],
                                               ecolor=self.parent.chart_tool.err_bar_color, bottom=bottom)
                    bottom += value_props[:, i]
                    if self.parent.chart_tool.bar_value_label_checked:
                        if len(self.parent.chart_tool.bar_value_label_series) < col_num:
                            self.parent.chart_tool.bar_value_label_series.extend(
                                (col_num-len(self.parent.chart_tool.bar_value_label_series))*[None]
